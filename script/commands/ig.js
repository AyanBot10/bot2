  ...
  try {
    ...
    case "-i":{
      ...
      if (!username) return api.sendMessage("id người dùng đâu", threadID);
       const res = await axios.get(`https://newapi2.faheem007.repl.co/instagram/infouser?ig=${username}`);
...
const res = await axios.get(`https://newapi2.faheem007.repl.co/instagram/downloadpost?url=${link}`);
var url = res.data.images[0].image_versions2.candidates[0].url;
    ...
    data = (await axios.get(`https://newapi2.faheem007.repl.co/instagram/downloadpost?url=${link_video}`)).data,
    ...
    axios.get(`https://newapi2.faheem007.repl.co/instagram/downloadpost?url=${link_post}`)/* request đến api */
