...
if (!text)
    return api.sendMessage("Add text lmao", event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://faheem-vip-010.faheem001.repl.co/api/maker/ttp?text=${text}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}
